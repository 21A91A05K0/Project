...
def summarize_text(text, num_words):
    """Summarizes text while preserving punctuation for readability."""
    try:
        num_words = int(num_words)
    except ValueError:
        return ["Invalid input: Please enter a numeric value."]
    ...
    def clean_text(text):
        """Removes stop words for keyword extraction."""
        stop_words = set(stopwords.words('english'))
        words = [word for word in word_tokenize(text) if word.isalnum() and word.lower() not in stop_words]
        return " ".join(words)

    # Clean the text for keyword extraction only
    cleaned_text = clean_text(text)

    # Process text with spaCy for efficient POS tagging
    doc = nlp(cleaned_text)

    # Extract keywords based on POS tags
    keywords = [token.text for token in doc if token.pos_ in ['PROPN', 'ADJ', 'NOUN', 'VERB']]

    # Calculate word frequencies and normalize
    freq = Counter(keywords)
    max_freq = freq.most_common(1)[0][1] if freq else 1
    for word in freq.keys():
        freq[word] = freq[word] / max_freq

    # Calculate sentence strengths based on keyword frequencies (using original sentences)
    sentences = sent_tokenize(text)  # Use original sentences with punctuation
    sent_strength = {sent: sum(freq.get(word, 0) for word in word_tokenize(sent)) for sent in sentences}
    ...
    word_count = 0
    for sentence in summarized_sentences:
        words = sentence.split()  # Split the original sentence with punctuation
        if word_count + len(words) <= num_words:
            ...
            remaining = num_words - word_count
            if remaining > 0:
                summary.append(" ".join(words[:remaining]) + "...")  # Add partial sentence with ellipsis
            break
